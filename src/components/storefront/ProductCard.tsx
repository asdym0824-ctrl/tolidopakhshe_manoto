...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div
      ...
      className="group bg-white rounded-2xl sm:rounded-3xl border border-[#EAE4D9] shadow-[0_2px_10px_rgba(24,24,27,0.03)] hover:shadow-[0_12px_28px_rgba(24,24,27,0.08)] hover:border-[#18181B] transition-all duration-300 flex flex-col overflow-hidden select-none"
    >
      {/* Image and Badges Header - Square on Mobile for Compact Visual Browsing */}
      <div 
        className="relative aspect-square sm:aspect-4/5 w-full bg-[#F6F3EC] overflow-hidden cursor-pointer active:opacity-95" 
      ...
      >
        ...
        <div className="absolute top-2 right-2 sm:top-2.5 sm:right-2.5 flex flex-col gap-1 sm:gap-1.5 items-end z-10">
          ...
          {isRetailAvailable ? (
            <span className="bg-[#967434]/95 backdrop-blur-xs text-white text-[8px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-md shadow-xs flex items-center gap-1">
        ...
        <div className="absolute bottom-1.5 right-1.5 left-1.5 sm:bottom-2 sm:right-2 sm:left-2 flex items-center justify-between pointer-events-none z-10">
          <span className="bg-white/95 backdrop-blur-xs text-stone-900 text-[8.5px] sm:text-[11px] font-bold px-1.5 sm:px-2 py-0.5 rounded-md shadow-xs truncate max-w-[70%] border border-[#EAE4D9]/80">
            {product.fabricType}
          </span>
          {product.rating && (
            <span className="bg-white/95 text-stone-900 text-[8.5px] sm:text-[11px] font-black px-1.5 sm:px-2 py-0.5 rounded-md shadow-xs flex items-center gap-0.5 sm:gap-1 border border-[#EAE4D9]/80">
        ...
        <div className="absolute inset-0 bg-stone-950/25 opacity-0 group-hover:opacity-100 transition-opacity hidden sm:flex items-center justify-center pointer-events-none">
          <span className="bg-[#FAF8F5] text-[#18181B] px-3.5 py-2 rounded-xl font-black text-xs shadow-lg flex items-center gap-1.5 transform translate-y-2 group-hover:translate-y-0 transition-transform border border-[#EAE4D9]">
            <Eye className="w-3.5 h-3.5 text-[#967434]" />
      ...
      <div className="p-2.5 sm:p-4 flex-1 flex flex-col justify-between space-y-2 sm:space-y-3">
        <div>
          {/* Category & SKU with Women Atelier Badge */}
          <div className="flex items-center justify-between text-[9px] sm:text-[11px] text-stone-500 mb-0.5 sm:mb-1">
            <div className="flex items-center gap-1 truncate">
              <span className="font-bold text-[#8C6D37] hover:text-stone-900 transition-colors cursor-pointer truncate">
                {product.category}
              </span>
              <span className="text-[8px] bg-rose-50 text-rose-800 border border-rose-200/80 px-1 py-0.2 rounded font-medium shrink-0">
                قواره زنانه
              </span>
            </div>
          ...
          <h3
            onClick={() => onOpenDetail(product)}
            className="font-black text-stone-900 text-[11.5px] sm:text-sm leading-snug line-clamp-1 sm:line-clamp-2 cursor-pointer hover:text-[#967434] transition-colors"
          >
            {product.name}
          </h3>

          {/* Quick Women Sizing preview */}
          {product.sizes && (
            <div className="flex items-center gap-1 mt-0.5 text-[8.5px] sm:text-[10px] text-stone-500 truncate">
              <Ruler className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-stone-400 shrink-0" />
                    ...
                    <button
                      ...
                      className={`text-[8.5px] sm:text-[10px] px-1.5 py-0.5 rounded transition-all cursor-pointer ${
                        isSel
                          ...
                          : 'bg-[#FAF8F5] text-stone-800 border border-[#EAE4D9] hover:border-stone-800'
                      }`}
                    >
                      {c}
                    </button>
        ...
        <div className="bg-[#FAF8F5] rounded-xl p-2 sm:p-2.5 border border-[#EAE4D9] space-y-1 sm:space-y-1.5">
          {/* Dual price mode mini pills */}
          {isRetailAvailable && (
            <div className="flex items-center bg-[#EDE7DC] p-0.5 rounded-lg text-[8.5px] sm:text-[10px] font-black">
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className={`flex-1 py-0.5 sm:py-1 rounded-md transition-all ${
                  activeMode === 'wholesale_pack' ? 'bg-[#18181B] text-[#FAF8F5] shadow-xs' : 'text-stone-700 hover:text-stone-900'
                }`}
              >
                پک عمده
              </button>
              <button
                ...
                className={`flex-1 py-0.5 sm:py-1 rounded-md transition-all ${
                  activeMode === 'retail_single' ? 'bg-[#967434] text-white shadow-xs' : 'text-stone-700 hover:text-stone-900'
                }`}
              >
                تک‌فروشی
              </button>
